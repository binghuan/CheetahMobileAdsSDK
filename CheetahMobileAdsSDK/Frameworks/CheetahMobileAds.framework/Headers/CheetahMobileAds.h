...
+ (void)startWithMIDConfig:(CMAMIDConfig *)MIDconfig isEUUser:(BOOL)isEUUser allowUserData:(BOOL)allowUserData;
+ (void)clearCache;
+ (void)personalizationEnabled:(BOOL)enabled;
+ (void)setEUUserEnabled:(BOOL)enabled;//20180522@BH_Lin
+ (NSString *)version;
